import requests
import random
from podcast_bot import PythonBytesBot
...
def make_reply(msg, name):
    ...
    results = (
        f"{random.choice(greetings)}, {name}! 👋 \n\nSearch for episodes from the <a href='https://pythonbytes.fm/'>Python Bytes</a> podcast based on a topic (example, <strong>async</strong>). To search with multiple words, separate them with a '-' (example, <strong>async-django</strong>)"
        if msg == "/start"
...
def get_results(msg):
    ...
    message_body_list = []
    message_body = ""
    message_heading = f"🐍⭐ Popular <strong><a href='https://pythonbytes.fm/'>Python Bytes</a></strong> episodes for <strong><em>{msg}</em></strong>:\n"

    r = requests.get(f"https://search.pythonbytes.fm/api/search?q={msg}")

    # Limit to the 15 most popular episodes only
    search_results = r.json()["episodes"][:15]

    for result in search_results:
        if result["category"] == "episode":
            message_body_list.append(
                f"\n\n🔸 <strong>Episode {result['item_id']}</strong> 〰️ <a href='{result['url']}'>{result['title']}</a>"
            )
...
def main():
    ...
    while True:
        print("Firing up the Python Bytes bot...")
